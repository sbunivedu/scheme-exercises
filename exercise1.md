...
<details>
<summary>Solution</summary>
The left child/subtree would be swapped with the right child/subtree on each
and every subtree.
</details>
...
(append '(a b) 'c); => (a b . c)
```
Try changing `(append (cdr lst) x)` to `(append x (cdr lst))`.
</details>
...
(shorter1 '(a b) '(c d e)); => (a b)
(shorter1 '(a b) '(c d)); => (a b)
(shorter1 '(a b) '(c)); => (c)
```
</details>
